...
pygame.init()
# Added Title, Icon
pygame.display.set_caption("Space Invader Game by Shraddha Ghawalkar")
...
def fire_bullet(x, y):
    global bullet_state
    bullet_state = "fire"
    screen.blit(bullet_image, (x + 16, y + 12))
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:                    
            if event.key == pygame.K_LEFT:                  # press LEFT Key to move player "Left"
                rocketX_change = -5
            if event.key == pygame.K_RIGHT:                 # press RIGHT Key to move player "Right"
                rocketX_change = 5
            if event.key == pygame.K_SPACE:                 # press SPACE Key to "Fire" bullets
